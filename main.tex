...
The categories of coordination approaches operate on different abstraction levels, namely \textit{execution}, \textit{model}, and \textit{language}.
Each category is distinct, i.e., approaches on a lower level are unrelated to approaches on a higher level.
...
A detailed explanation of each category, including examples, is given in the following subsections.
...
\textit{A co-simulation approach} composes the simulation of multiple components into a global simulation \cite{gomesCoSimulationSurvey2019}.
...
\textit{A coordination language} provides a set of operations and infrastructure to communicate and synchronize between components potentially executing on different machines.
Usually, coordination languages are embedded into programming languages, effectively adding coordination operations to the language.
Coordination languages are described in detail in \autoref{subsec: coordlang}.

% Model level
\textbf{Model level:}

\textit{Architecture Description Languages (ADLs)} operate on the \textit{model} level, i.e., each \textit{component} is given by a behavioral model, for example, a state machine.
...
\textit{Coordination frameworks} allow the user to connect models conforming to \textit{different} behavioral languages using the concept of \textit{behavioral interfaces}.
...
Coordination Languages traditionally offer a shared tuple space for concurrent programs to exchange data (see execution level in \autoref{fig: overview}).
...
% TODO: Motivation is here again. Should be changed and more fitting
...
It can be used before the system is deployed (\textit{model checking}) or while it is running (\textit{runtime monitoring}).
...
For example, Linda is implemented in multiple languages such as C, C\textsf{++}, Java, and more, while Ptolemy is implemented in Java \cite{ptolemaeusSystemDesignModeling2014}. 

As an alternative, a coordination approach can be implemented in a \textit{formal language} which can then be run on an execution engine implemented for that formalism.
...
Federated execution can still happen on the same physical machine, i.e., does not mean that components must be \textit{distributed} across different physical machines 
...
DACCOSIM \cite{galtierFMIBasedDistributedMultisimulation2015} goes one step further and even allows a \textit{distributed} federated execution.
...
From \autoref{fig: venn-bcoorlang-montiarc-linda}, one can see that Linda has three original features when compared to BCoorLang and MontiArc.
...
For example, a difference in the \textit{domain} feature might be more profound than a difference in another feature.
...
Jaccard distance is the one-complement of Jaccard similarity for two sets, which is defined as the ratio of their intersection to their union  \cite{levandowskyDistanceSets1971}.
It is a widely used and reasonable measure of dissimilarity for sets \cite{levandowskyDistanceSets1971}.
