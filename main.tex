...
\newcommand{\jdChange}[2]{
{\color{green!20}JD:} {\color{gray}\sout{#1}} {\color{mygreen}
{#2}}
}
...
% Query-based example
For example, in BCOoL, one can write queries to define which elements should coordinate~\cite{varalarsenBCOolBehavioralCoordination2016,varalarsenBehavioralCoordinationOperator2015}.
...
The update of an output can cause reactions, i.e., state changes in the input of the other systems.
...
Due to space constraints, we cannot show all classifications, but the complete data set is available in the artifacts of this paper~\cite{timkrauterArtifactsCoordination2024}.
...
It is intriguing how a single approach like Ptolemy can encompass a wide variety of features.
...
% Future work
In future work, we aim to refine our taxonomy and apply it to more approaches to obtain an even more comprehensive overview of coordination approaches.
