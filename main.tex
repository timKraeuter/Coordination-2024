...
The feature model allows to compare coordination approaches across the different categories while previous research has only focused on comparison inside categories. 
Then, we apply the feature model to evaluate 16 distinct approaches spanning the aforementioned categories.
Finally, we discuss our findings highlighting the common and distinct features among the categories, while also identifying crucial features that are absent but essential for industry application in the future.
% TODO: Add something more concrete about findings once we have written the findings section.
