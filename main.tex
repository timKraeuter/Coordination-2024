...
% Contribution
\textbf{Contribution.} We present a \textit{feature model} \cite{kangFeatureOrientedDomainAnalysis1990} to classify coordination approaches which allows the comparison of coordination approaches across the different categories. Previous research has only focused on comparing approaches inside the categories not considering a broader scope. 
...
\section{Feature model} \label{sec:features} % This could be called taxonomy which is represented as a feature model
