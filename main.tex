...
% Contribution
\textbf{Contribution.} We present a feature model comparing coordination approaches from different categories, namely coordination languages, ADLs, co-simulation tools, and coordination frameworks.

% Paper outline
\textbf{Outline.} The remainder of the paper is structured as follows.
First, we introduce and describe the different categories of coordination approaches (\autoref{sec:approaches}).
Afterward, we present our main contribution: the feature model (\autoref{sec:features}).
Then, we apply the feature model to four representative from each category of coordination approaches in \autoref{sec:application}.
...
The behavior of the \textsf{PedestrianTL} and \textsf{CarTL} component is omitted but given in \cite{timkrauterArtifactsCoordination2024}.
...
% We try to find overlaps and unique features (differentiators) between the different approaches.
...
% TODO: Why did we choose these? Representative feature sets? Recent publciations? --> Not Linda
However, in total we classified 16 approaches including the four representatives chosen.
...
From \autoref{fig:venn-Linda} one can see that most Linda features are overlapping with either BCoorLang or MontiArc.
Only \textsf{Execution} as a goal and \textsf{Definition:Name based} for coordination are not shared.
This makes sense since Linda and other initial coordination languages have greatly influenced the development of ADLs and coordination frameworks which happened afterwards.
...
Multiple unique features such as heterogeneous component languages and an underlying implementation in a formal language which permit formal verification exist,
...
% ADLs, coordination languages and coordination frameworks usually apply to software systems, while co-simulation applies to hybrid systems, i.e., cyber-physical systems. Thus, they support different MoC, domains and have different goals.
...
This is most likely due to data exchange being difficult to achieve when heterogeneous components are allowed.
However, data exchange is omnipotent in real-world systems, which is why it is the main coordination mechanism used in co-simulation tools and coordination languages.
