...
\textit{Event-scheduling coordination}, used in BCOoL and BCoorLang, orders or synchronizes events/state changes happening in different models.
...
% What is a model of computation? Where does the terminology come frome?
We define three MoCs: \textit{timed}, \textit{causal}, \textit{a-causal}, where \textit{timed} is either \textit{physical} or \textit{logical}.
% We should define that event and state changings elements mean the same, i.e., any observable action in a model/component.
...
For example, a delay could be 50 milliseconds of logical time which corresponds to time in a simulation which can run arbitrarily fast. 
% Timed::Physical
...
% Logical time however always lacks behind physical time.


\subsection{Goal} % Or even call it motivation
We identified three different goals for coordination approaches: \textbf{coordinated simulation}, \textbf{coordinated execution}, and \textbf{formal verification}, see \autoref{fig:coordinationFeature}.
...
% TODO: Introduce somewhere that component is equal to behavioral model. Or means the same.
\subsubsection{Coordinated simulation} The most common goal of coordination approaches is a coordinated simulation of multiple components or behavioral models which when composed represent the entire system.
A coordinated simulation is useful because composition of multiple component can lead to unexpected behavior, often called \textit{emergent behavior} \cite{ekerTamingHeterogeneityPtolemy2003}.
Emergent behavior can then be dealt with early in the development process before each component and their interactions are implemented.
Co-Simulation tools are an example of coordination approaches which have coordinated simulation as their goal.
