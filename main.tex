...
Such coordination languages are often called \textit{tuple-based} because tuples are exchanged through the shared memory \cite{rossiTuplebasedTechnologiesCoordination2001,omiciniCoordinationModelsLanguages2011}.
...
\section{Methodology}
% Tertiary study first: Cite all surveys.
% one search with ADL, coordination language, co-simulation
% first three pages

% Deeper study based on distribution across the categories, the knowledge of the authors (significant differences between approaches, mixing old and recent approaches), and time available.
