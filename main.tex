...
% TODO: Rewrite this
\subsubsection{Component languages} Coordination languages typically only support coordination if each component is written in the same programming language (\textit{homogeneous}).
...
\subsubsection{Implementation} Coordination approaches are generally implemented in two different ways.
First, in a \textit{co-simulation} each component runs as an independent black box.
Then, a coordination approach provides an \textit{orchestrator} which controls passage of time and moves output data from one component to another \cite{gomesCoSimulationSurvey2019}, i.e., facilitates coordination.
...
A distributed, i.e., federated execution is in the early stages of development for Lingua Franca but not officially supported\footnote{\url{https://www.lf-lang.org/docs/writing-reactors/distributed-execution}} yet which is why we did not take it into consideration for our feature model.
The coordination framework BCoordLang \cite{krauterBehavioralConsistencyMultimodeling2023} uses graph or term-rewriting as formal languages to coordinate heterogeneous components.
