...
Examples of coordination frameworks are BCOoL \cite{varalarsenBCOolBehavioralCoordination2016,varalarsenBehavioralCoordinationOperator2015} and BCorrLang \cite{krauterBehavioralConsistencyMultimodeling2023}.
% BCorrLang is new. A bit close to BCOoL

% Example using bCoorLang
\autoref{lst:interactions} shows how the running example can be implemented in BCorrLang.
...
\subsection{MontiArc} \label{subsec:montiArc}% ADL representative

To implement the running example, we define a component for the \textsf{PedestrianCrossing} with two subcomponents for each type of TL.
...
% Maybe explain state and state-changing elements or drop it already earlier.
% Maybe explain how BCorrLang does the synchronization using rule merges
