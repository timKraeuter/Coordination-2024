...
% Query-based example
As an example, in BCOoL \cite{varalarsenBCOolBehavioralCoordination2016,varalarsenBehavioralCoordinationOperator2015}, one can write expressions to define which elements should coordinate.
A structural definition of coordination is using \textsf{Dedicated elements} or \textsf{Architecture based}.
...
\textsf{Event-Scheduling coordination}, for example used in BCOoL and BCoorLang, orders or synchronizes events/state changes happening in different models.
\textsf{Data-Driven coordination} is for example used in Co-Simulation where components send data to input ports or even directly change variables exposed by other components.
...
For example in BCOoL, user can employ the Clock Constraints Specification Language (CCSL) \cite{andreSyntaxSemanticsClock2009} when defining coordination \cite{varalarsenBCOolBehavioralCoordination2016,varalarsenBehavioralCoordinationOperator2015}.
