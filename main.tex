...
Examples of coordination frameworks are BCOoL \cite{varalarsenBehavioralCoordinationOperator2015,varalarsenBCOolBehavioralCoordination2016} and BCoorLang \cite{krauterBehavioralConsistencyMultimodeling2023}.
% Ptolemy?
...
\subsubsection{Definition} of coordination can either be \textsf{Explicit} or \textsf{Implicit}.
Implicit definition is based on the names of model elements.
...
% Query-based example
As an example, in the Behavioral Coordination Operator Language (BCOoL) \cite{varalarsenBehavioralCoordinationOperator2015,varalarsenBCOolBehavioralCoordination2016} one can write expressions to define which elements should coordinate.
A structural definition of  coordination can be based on \textsf{Dedicated elements} or the architectural configuration, often-called topology, of components (\textsf{Architecture based}).
For example, interactions in BCoorLang are specified by dedicated elements, while coordination in ADLs depends on the architecture, i.e., how ports of components are connected.

\subsubsection{Elements of coordination} describe Elements of coordination here.

\subsubsection{Semantics} describe Semantics here.
